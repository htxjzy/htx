...
form.on('checkbox(expCheckbox)', function(data){			
	$("input[name='like[]']").attr('disabled', true);
   	if ($("input[name='like[]']:checked").length >= 5) {
...
//Executing an uploaded images instance-for-qualification-name
upload.render({
	elem: '#qualifi-name-upload' //Binding element
    ,url: '/submission_upload/upfilesproccess.php?uploadact=images' //Upload interface
	,size: 1000 //Limit the size of the picture, unit KB
	,before: function(obj){
      //According to the local picture, does not support IE8
      obj.preview(function(index, file, result){
        $('#locaimg-qualifi-name').attr('src', result); //Picture linking（base64）
      });
    }
    ,done: function(res){
      //Upload finished callback
	  ...
	  //layer.msg(JSON.stringify(res));
	  var imgUrl="/submission_upload/upfiles/"+res.message;
	  $("input[name='p_qualification_name']").attr('value', imgUrl); 
...
//Executing an uploaded images instance-for-qualification-cont
upload.render({
	elem: '#qualifi-cont-upload' //Binding element
    ,url: '/submission_upload/upfilesproccess.php?uploadact=images' //Upload interface
	,size: 1000 //Limit the size of the picture, unit KB
	,before: function(obj){
      //According to the local picture, does not support IE8
      obj.preview(function(index, file, result){
        $('#locaimg-qualifi-cont').attr('src', result); //Picture linking（base64）
      });
    }
    ,done: function(res){
      //Upload finished callback
	  ...
	  //layer.msg(JSON.stringify(res));
	  var imgUrl="/submission_upload/upfiles/"+res.message;
	  $("input[name='p_qualification_cont']").attr('value', imgUrl); 
...
upload.render({
	elem: '#qualifi-upload' //Binding element
    ,url: '/submission_upload/upfilesproccess.php?uploadact=images' //Upload interface
	,size: 1000 //Limit the size of the picture, unit KB
	,before: function(obj){
      //According to the local picture, does not support IE8
      obj.preview(function(index, file, result){
        $('#locaimgzz-qualifi').attr('src', result); //Picture linking（base64）
      });
    }
    ,done: function(res){
      //Upload finished callback
	  ...
	  //layer.msg(JSON.stringify(res));
	  var imgUrl="/submission_upload/upfiles/"+res.message;
	  $("input[name='c_enterprise_qualification']").attr('value', imgUrl); 
...
upload.render({
	...
	var submitUrl = "/other/shop_come_in";
