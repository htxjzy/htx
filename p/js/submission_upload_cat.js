...
var uploadInst = upload.render({
    ...
    ,done: function(res){
	  ...
	  $("input[name='ass_upimg']").attr('value', imgUrl); 
...
upload.render({
	elem: '#zip-upload' //Binding element
    ,url: '/submission_upload/upfilesproccess.php?uploadact=file' //Upload interface
	,size: 2000 //Limit the size of the picture, unit KB
	,before: function(obj){
      //According to the local file, does not support IE8
      obj.preview(function(index, file, result){
		//
		$("#zip-upload").parent().append('<span style="margin-left:4px;">'+file.name+'</span>');		
      });
    }
    ,accept: 'file' //Ordinary file
    ,exts: 'zip|rar|7z' //Only upload compressed file
    ,done: function(res){
      //Upload finished callback
      //If the upload failed
      if(res.status > 0){
	  ...
	  $("input[name='ass_upzip']").attr('value', zipUrl); 	  
		...
		if(data.value==0){			
			...
			form.on('select(subcat)', function(data){
				$.ajax({
					...
					type: 'get',
					success: function(data) {											
						$(".dynimcBox").empty().html(data+'<div class="layui-form-item"><label class="layui-form-label">服务费预算(元)</label><div class="layui-input-block"><input type="text" name="ass_fei" required  lay-verify="required|positiveInteger" placeholder="请根据行业标准预估项目服务费用" autocomplete="off" class="layui-input"></div></div>');																		
					}
				});				
			});
...
//must_address start
	...
	function treeSelect(config) {
		config.v1 = config.v1 ? config.v1 : 910000;
		config.v2 = config.v2 ? config.v2 : 910100;
		config.v3 = config.v3 ? config.v3 : 910101;
		
		$("<option value='910000'>不限</option>").appendTo($form.find('select[name=' + config.s1 + ']'));
		...
		function cityEvent(data) {
			...
			$("<option value='910100'>不限</option>").appendTo($form.find('select[name=' + config.s2 + ']'));		
			$.each(threeSelectData, function (k, v) {
				if (v.val == config.v1) {
					if (v.items) {
		...
		function areaEvent(data) {
			...
			$("<option value='910101'>不限</option>").appendTo($form.find('select[name=' + config.s3 + ']'));
			$.each(threeSelectData, function (k, v) {
				if (v.val == config.v1) {
					if (v.items) {
						$.each(v.items, function (kt, vt) {
			...
			success:function(rdata) {
				...
				layer3.open({
					...
					yes: function(index, layero){
						...
						window.location.href="/other/i_want_to_recharge"; 
						layer.close(index); 
					},
