...
$(function () {
	...
	/** 数字 + 字母组合，长度 6~16 */
	//var regexPassword = /^(?![^a-zA-Z]+$)(?!\D+$).{6,16}$/;
    ...
    $.ajax({
        ...
        success: function(data) {					
			...
			layui.use('layer', function(){ 
				var $ = layui.$ 
				,layer = layui.layer;				
				layer.msg(returnmsg, { time:2000, shade:[0.7, '#393D49'] }, function(){
...
///////////////////////////////Login页面使用Js-buyaoheshangmianguanlian///////////////////////////////
...
//Verify whether the phone number or the mailbox is correct when the account is logged in
    ...
    SignUp.IsEmailOrPhone = function () { 
		...
		if( !regexEmail.test(eOrp) && !regexPhone.test(eOrp)){
			$("#errordiv2").removeClass("layui-hide");
			$("#EmailOrPhone").addClass("error");
			$("#errortip2").text(emailOrPhoneError);
			bol = false;		
		}
	...
	/*手机密码验证-LOGIN*/
	SignUp.CheckPasswordLog = function () {
		
		$("#errortip2").text('');
		$("#txtPassword7").removeClass("error");
		$("#errordiv2").addClass("layui-hide");
	
		var password = $("#txtPassword7").val().trim();
		$("#errortip2").text('');
		
	
		if (password == "" || password == null || password == undefined) {
			$("#errordiv2").removeClass("layui-hide");
			$("#txtPassword7").addClass("error");
			$("#errortip2").text(passwordEmpty);
			return false;
		}
		...
		if (!regexPassword.test(password)) {
			$("#errordiv2").removeClass("layui-hide");
			$("#txtPassword7").addClass("error");
			$("#errortip2").text(passwordFormatError);
			return false;
		}
    ...
    /*SignUp.CheckPasswordSubmit = function () {
        ...
        if (!SignUp.CheckPasswordLog()) return false;
            ...
            success: function (data) {
				...
				layui.use('layer', function(){ 
					...
					layer.msg(returnmsg, { time:3000, shade:[0.7, '#393D49'] }, function(){
					  window.location.href="/other/user_center"; 
					});		 
				});			
            }
        });
...
//before post form validate for normal login
function logcheck(){
...
//before post form validate for quick login
