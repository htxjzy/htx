...
if(!empty($_POST['like']) && isset($_POST['whosedata'])){	
	...
	if($whosedata=="per"){
		$shop_meta_arr = array(			
			...
			'_shop_pc_qualifi_name'  	=> $_POST['p_qualification_name'],
			'_shop_pc_qualifi_cont'  	=> $_POST['p_qualification_cont'],
			
			'_htx_shop_name'  			=> wp_strip_all_tags( $_POST['shop_name'] ),
			'_htx_shop_logo'  			=> $_POST['logo_img'],			
	...
	if($whosedata=="cor"){
		$shop_meta_arr = array(			
			...
			'_shop_enterprise_qualifi'  => $_POST['c_enterprise_qualification'],	
	...
	if( 0 !== wp_insert_post($shop_post)){
		echo '{"reg":"您好，店铺资料已提交，审核过后即发布 <i class=\"layui-icon\" style=\"font-size:30px; color:#FF5722;\">&#xe60c;</i>"}';		
