...
if($session) {
    ...
    echo 'B 您好:' . $session['name'] . '<a href="http://www.htxgcw.cn/sso/session-api.php?action=logout&sessid='.$sessid.'">退出</a>';
	echo '<br/>';
	echo $session['name'].'-'.$session['passwd'].'-'.$session['email'].'-'.$session['phone'];
	
} else {
    echo '<span id="name">B您还没有登录!<a href="http://www.htxgcw.cn/login.php">去登录</a></span>';
}
