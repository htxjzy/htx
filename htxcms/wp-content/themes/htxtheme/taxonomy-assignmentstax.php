		...
		$(".all_region").hide(200);
                            ...
                            <table>
                                ...
                                <tr>
                                    ...
                                    <th>任务模式-状态 <br/> 距投标截止期</th>
                                    <th>我要操作</th>
                                </tr>
                                </thead>
                                <tbody>
<?php 
$cur_url = home_url().'/assignmentstax/'.$cur_slug;
...
$args=array(
	...
	'paged' 	=> $paged,
	'tax_query' => array(array('taxonomy' => 'assignmentstax', 'field'=>'slug', 'terms' =>$cur_slug))        
);
...
<div class="htx-deputy">
    <h3>在<?php echo $name; ?>方面，提供优质服务的服务商代表</h3>
	<div class="htx-deputy-box layui-clear">
<?php
$args=array(
...
if($recentPosts->have_posts()){
	for($i=0; $i<7; $i++){
		...
		if(!empty(get_post_meta( $post->ID, '_htx_shop_name', true ))){
?>        
        <a href="<?php the_permalink(); ?>" title="<?php $shop_name=get_post_meta( $post->ID, '_htx_shop_name', true ); echo $shop_name; ?>" target="_blank"><img src="<?php echo get_post_meta( $post->ID, '_htx_shop_logo', true ); ?>" /></a>
