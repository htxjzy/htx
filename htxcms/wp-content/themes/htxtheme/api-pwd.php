<?php
		...
		$post="act=SettingPassword_GCW&name={$name}&passwd={$passwd}&email={$email}&phone={$phone}&redirect={$redirect}";	
		vcurl($url, $post);
		
		$url2="http://job.htxgcw.com/members/sso";
		$post2="flag=pwd03&name={$name}&passwd={$passwd}&email={$email}&phone={$phone}&redirect={$redirect}";	
		vcurl($url2, $post2);*/
		//以上先屏蔽掉，网上正式上线后在打开
